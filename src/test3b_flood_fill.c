...
int main(int argc, char *argv[]) {
  ...
  for(j=0;j<5;j++) {
    ...
    // Set a random fill color for flood fill
    fillColor.c[0] = drand48();
    fillColor.c[1] = drand48();
    fillColor.c[2] = drand48();

    // Apply flood fill inside the box
    flood_fill(src, (x0 + x1) / 2, (y0 + y1) / 2, fillColor);
  ...
  // write the image
  image_write( src, "test3b_flood_fill.ppm");
